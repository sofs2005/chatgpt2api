...
var apiPermissionCatalog = []APIPermission{
	...
	apiPermission("POST", "/v1/ppt/generations", "PPT 生成", "创作", false),
	apiPermission("POST", "/v1/psd/generations", "PSD 生成", "创作", false),
	apiPermission("GET", "/v1/editable-file-tasks", "查看可编辑文件任务", "创作", false),
...
func DefaultPermissionSetForRole(role string) PermissionSet {
	...
	return PermissionSet{
		...
		APIPermissions: NormalizeAPIPermissions([]string{
			...
			APIPermissionKey("POST", "/v1/ppt/generations"),
			APIPermissionKey("POST", "/v1/psd/generations"),
			APIPermissionKey("GET", "/v1/editable-file-tasks"),
